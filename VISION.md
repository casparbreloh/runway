...
1. **Manual cloud run:** implement content-addressed local-tree Source, automatic Stack provisioning,
   durable run admission and Terminal status, redacted log streaming, cancellation, and
   `runway run <workflow>`. Reject workflow secrets and cache publication for untrusted local Source.
   This makes the workflow generated by `runway init` executable.
2. **GitHub connection and publication:** implement `runway connect github`, configure Cloudflare
   Workers Builds for the default branch, publish exact Source and immutable workflow artifacts, and
   atomically activate a repository workflow registry. Neither `deploy` nor `publish` becomes normal
   user UX.
3. **Conditional GitHub App:** only when `github()` is declared, use the GitHub App Manifest flow,
   direct webhooks to the user's Worker, retain credentials only in the user's Cloudflare account,
   and mint short-lived repository- and permission-scoped installation tokens. Delete the current
   manual App credential setup rather than preserving compatibility.
4. **Evidence and release gates:** prove the exact local-run and connected-GitHub paths live, then
   build same-source runtime cohorts and comparative benchmarks before making release claims.
