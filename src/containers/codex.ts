...
/**
 * Container running the OpenAI Codex CLI. Exposes the SandboxService surface
 * (exec/writeFile/readFile/setEnvVars) as RPC methods. The Dockerfile installs
 * only git, gh, and the codex CLI to keep the image minimal.
 */
export class CodexContainer extends Cloudflare.Container<
  CodexContainer,
  {
...
export const CodexContainerLive = /* @__PURE__ */ CodexContainer.make(
  Effect.gen(function* () {
    const cp = yield* ChildProcessSpawner;
    const fs = yield* FileSystem.FileSystem;

    // Spawned-process env: seed from the container's own env (PATH, HOME, ... so
    // git/gh/codex resolve) and merge in whatever setEnvVars adds.
    const env: Record<string, string> = {};
    for (const [key, value] of Object.entries(process.env)) {
      if (value !== undefined) env[key] = value;
    }

    return CodexContainer.of({
      exec: (command) =>
      ...
      fetch: Effect.succeed(HttpServerResponse.text("codex container")),
    });
  }),
);

export default CodexContainerLive;
