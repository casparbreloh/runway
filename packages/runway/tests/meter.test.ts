...
test("meter reports emit as one queryable JSON record", () => {
  const log = vi.spyOn(console, "log").mockImplementation(() => {});
  const report = {
    schema: 1 as const,
    priceTableId: "fixture",
    samples: [{ type: "tool", state: "prepared", count: 1, durationMs: 7 }],
    estimate: { priceTableId: "fixture", usd: 0 },
  };

  try {
    emitMeterReport(report);
    expect(log).toHaveBeenCalledOnce();
    expect(JSON.parse(String(log.mock.calls[0]?.[0]))).toEqual({ type: "runway-meter", report });
  } finally {
    log.mockRestore();
  }
});
