...
export const decodeWorkflowArtifact = (bytes: ArrayBuffer): WorkflowArtifact => {
  ...
  if (
    ...
    typeof value !== "object" ||
    Array.isArray(value) ||
    Object.keys(value).sort().join(",") !== "scriptName,secrets,source,workflowId"
  ) {
    throw new Error("invalid workflow artifact");
  }
  const { scriptName, workflowId, secrets, source } = value as Record<string, unknown>;
  if (
  ...
  return { scriptName, workflowId, secrets, source } as WorkflowArtifact;
};
